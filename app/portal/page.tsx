...
const FeatureItem: React.FC<FeatureItemProps> = ({ icon: Icon, title, description, delay }) => (
  ...
  >
    ...
    <div className="flex-1 break-words whitespace-normal">
      <h3 className="text-lg font-serif font-bold text-transparent bg-clip-text bg-gradient-to-r from-red-400 via-amber-300 to-yellow-200">
        {title}
      </h3>
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="relative mb-8 flex flex-col items-center rounded-xl border border-red-500/30 
                   bg-gradient-to-tr from-slate-900/70 to-slate-950/40 p-6 w-full max-w-2xl mx-auto text-center shadow-2xl"
      >
        <FieryArrowLogo />
        <h1 className="mt-4 text-3xl sm:text-4xl font-bold text-transparent bg-clip-text 
      ...
      <motion.div
        ...
        className="relative w-full max-w-2xl mx-auto p-6 rounded-2xl border border-red-500/30 
                   bg-slate-900/70 backdrop-blur-md shadow-2xl overflow-visible"
        style={{ WebkitOverflowScrolling: 'touch' }}
      >
        <FlameWisps />
        <GoldenBubbles />
        <div className="relative z-10">
          <h2 className="mb-6 text-center text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-red-400 via-amber-300 to-yellow-200 font-serif">
            Your Arsenal of Abilities
            <Sparkles className="inline-block ml-2 text-yellow-300 h-7 w-7 animate-pulse-slow" />
          </h2>

          {/* Make the list area pad the bottom so final item never gets cut */}
          <motion.ul
